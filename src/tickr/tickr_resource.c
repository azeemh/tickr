...
/* OLD FORMATTING SECTION ORIGINAL 0.7.1 code
...
// added 0.7.0 code to restore html_entities to be used again

/* 0.7.0 restored formatting that works but has a blank issue
 //* Actual 'formatting' is done here.
 //* str must have been created with l_str_new() (or malloc()).
 
char *format_resource_str(char *str)
{
#define	ENTITY_LEN	15
	char 		*str2, *s, *d;
	char		entity[ENTITY_LEN + 1];
	zboolean	is_inside, strip_tags, entity_found;
	int		i, j;
	char		c;
	...
	*d = '\0';
	l_str_free(str);
	str = str2;
	//'Translate' html entities Libxml2 html named entities issue (not a Tickr bug): Libxml2 reads html *numerical* entities, not *named* ones (with a few exceptions) so, for instance, &#160; is OK but &nbsp; is not.
	str2 = l_str_new(str);
	...
	for (s = str, d = str2; s < str + strlen(str) && d < str2 + strlen(str2) - ENTITY_LEN;) {
		...
		if (c == '&') {
			...
			} while ((c = *s++) != '\0' && i < ENTITY_LEN);
			entity[i] = '\0';
			if (entity_found) {
				//If entity is numerical, we need to remove any leading '0' in numerical value before comparing with strings in html_entity array.
				if (entity[1] == '#' && entity[2] == '0') {
					j = atoi(entity + 2);
					str_n_cpy(entity + 2, itoa2(j), ENTITY_LEN - 1);
					str_n_cat(entity, ";", 1);
				}
				j = 0;
				while (html_entity[j] != NULL) {
					if (strcmp(entity, html_entity[j]) == 0 ||
							strcmp(entity, html_entity[j + 1]) == 0) {
...
#define ENTITY_LEN 15
    ...
    *d = '\0';
     ...
     */
    str2 = l_str_new(str);
    str2 = realloc2(str2, (strlen(str) * 2) + TMPSTR_SIZE);

    for (s = str, d = str2; *s != '\0'; ) {
        c = *s;
        ...
        if (c == '&') {
            ...
            if (!entity_found) {
                /* Not a valid entity, just output '&' */
                *d++ = '&';
                /* s already points to next char after '&' */
                continue;
            }
            
            /* Handle numeric entities with leading zeros */
            if (entity[1] == '#' && entity[2] == '0') {
                j = atoi(entity + 2);  /* Skip "&#0" */
                snprintf(entity + 2, ENTITY_LEN - 2, "%d", j);
                strcat(entity, ";");
            }
            
            /* Look up entity in table */
            j = 0;
            while (html_entity[j] != NULL) {
                if (strcmp(entity, html_entity[j]) == 0 ||
                    strcmp(entity, html_entity[j + 1]) == 0) {
                    /* Found it - copy replacement */
                    strcpy(d, html_entity[j + 2]);
                    d += strlen(html_entity[j + 2]);
                    break;
                }
                j += 3;
            ...
            if (html_entity[j] == NULL) {
                /* Entity not found - output original */
                strcpy(d, entity);
                d += strlen(entity);
            }
        } else {
