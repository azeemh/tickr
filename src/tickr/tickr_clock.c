...
#ifndef G_OS_WIN32
//#  define NO_PADDING		"-"			/* Don't pad a numeric result string. The %- flag is a Glibc extension, not ISO C, so expect a compile-time warning. */
#  define NO_PADDING		""			/* Don't pad a numeric result string. The %- flag is a Glibc extension, not ISO C, so expect a compile-time warning. */
...
#define DATE_STR		"%a %b %d  "	/* Mon Jan 01 */
//#define DATE_STR_2		"%a %" NO_PADDING "d %b  "	/* Mon 01 Jan */
#define DATE_STR_2		"%a %d %b  "	/* Mon 01 Jan */
